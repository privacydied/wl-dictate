...
def test_wtype_passes_delay_and_uses_stdin(monkeypatch):
    cap = _Capture()
    monkeypatch.setattr(subprocess, "run", cap)
    WtypeEmitter(delay_ms=6).emit("hello world.")
    cmd, kwargs = cap.calls[0]
    assert cmd == ["wtype", "-d", "6", "-"]
    # Text goes on stdin, not argv — so a leading "-" can't be read as a flag.
    assert kwargs["input"] == "hello world."


def test_wtype_zero_delay_omits_flag(monkeypatch):
    cap = _Capture()
    monkeypatch.setattr(subprocess, "run", cap)
    WtypeEmitter(delay_ms=0).emit("hi")
    cmd, _ = cap.calls[0]
    assert cmd == ["wtype", "-"]
...
def test_make_emitter_threads_delay(monkeypatch):
    monkeypatch.delenv("WL_DICTATE_EMIT", raising=False)
    e = make_emitter("commit", wtype_timeout_s=5.0, wtype_delay_ms=12)
